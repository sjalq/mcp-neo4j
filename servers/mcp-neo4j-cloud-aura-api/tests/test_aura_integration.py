...
pytestmark = pytest.mark.skipif(
    not os.environ.get("NEO4J_AURA_CLIENT_ID")
    or not os.environ.get("NEO4J_AURA_CLIENT_SECRET"),
...
def aura_client():
    ...
    if not client_id or not client_secret:
        pytest.skip(
            "NEO4J_AURA_CLIENT_ID and NEO4J_AURA_CLIENT_SECRET environment variables are required"
        )
...
def test_integration_flow(aura_client, test_type):
    ...
    if (
        test_type == "create_instance"
        and os.environ.get("ENABLE_INSTANCE_CREATION") == "true"
    ):
        ...
        try:
            ...
            updated = aura_client.update_instance(
                instance_id=instance_id, name=updated_name
            )
            ...
            instance_details = wait_for_instance_status(
                aura_client, instance_id, "running"
            )
            ...
            instance_details = wait_for_instance_status(
                aura_client, instance_id, "paused"
            )
            ...
            assert "status" in instance_details
            print(
                f"Deleted test instance {instance_id}: {delete_result} {instance_details}"
            )
